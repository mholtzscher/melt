...
import type { FlakeInput, AppView, UpdateStatus } from "../lib/types";
import {
  hasFlakeNix,
  getFlakeMetadata,
  updateInputs,
...
export function AppProvider(props: { children: JSX.Element }) {
  ...
  const actions: AppActions = {
    ...
    async lockToCommit(
      ...
      setState("statusMessage", `Locking ${inputName} to ${sha.substring(0, 7)}...`);

      const result = await lockInputToRev(inputName, sha, owner, repo, flakePath);

      if (result.success) {
        setState("statusMessage", `Locked ${inputName} to ${sha.substring(0, 7)}`);
        await actions.refresh();
        setTimeout(() => setState("statusMessage", undefined), 3000);
        return true;
      } else {
